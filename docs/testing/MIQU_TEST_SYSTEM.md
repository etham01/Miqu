...
```
   ...
   ┌──────────────────────┼──────────────────────┬──────────────────────┐
   ↓                      ↓                      ↓                      ↓
 YAML 用例文档        JUnit (291)            pytest (248)          浏览器回归 (12)
...
| pytest 用例 | **248** |
| 浏览器回归断言 | **12** |
| 自动化用例合计 | **539**（不含浏览器层）/ **551**（含） |
| pytest 覆盖的接口 | 50/50（重构后 100%） |
| YAML 登记的核心规则 | 见 `test_cases/` |

最近一次执行：**539 条全部通过，0 失败**（另加浏览器回归 12/12）。详见
...
已验证（2026-09-14 连续两次执行，均为 `248 passed`）。
